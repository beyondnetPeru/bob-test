import { Monitor, Weight } from 'lucide-react';
        ...
        <table className="w-full min-w-[1320px] border-collapse text-left text-sm">
          <thead>
            <tr className="border-b border-zinc-800/50 bg-zinc-900/30">
              <th className="px-4 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-500">
                Computer
              </th>
              <th className="px-4 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-500">
                Category
          ...
          <tbody className="divide-y divide-zinc-800/30">
            {items.length === 0 && (
              <tr>
                ...
                >
                  No computer products match the current search and filters.
                </td>
              </tr>
            )}
            {items.map((item) => (
              ...
              >
                ...
                <td className="px-4 py-4 text-zinc-300">{item.cpu || '-'}</td>
                <td className="px-4 py-4 text-zinc-300">{item.ram || '-'}</td>
                <td className="px-4 py-4 text-zinc-300">
                  {item.hardDisc || '-'}
                </td>
                <td className="px-4 py-4 text-zinc-400">{item.ports || '-'}</td>
                <td className="px-4 py-4 text-zinc-300">{item.video || '-'}</td>
                <td className="px-4 py-4">
